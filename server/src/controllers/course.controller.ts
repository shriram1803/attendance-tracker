...
export const incrementAttendance = async (req: Request, res: Response) => {    
    try {
        ...
        if (!course) {
            return res.status(401).json({ message: 'Course not available' });
        }

        res.status(200).json({message: "Incremented attendance count by 1"});
    } catch(err) {
        res.status(500).json({error: "Could not update attendance detail"});
    }
};
...
export const decrementAttendance = async (req: Request, res: Response) => {    
    try {
        ...
        res.status(200).json({message: "decremented attendance count by 1"});
    } catch(err) {
        res.status(500).json({error: "Could not update attendance detail"});
    }
};